...
| The PIN never leaves this process, never enters a GVariant, a GError or a log line | `src/ui/pin.c` | there is no entry point that returns a PIN; the caller passes a login function. **On the `system` prompter path it ARRIVES from one**: the shell's prompter collects it and gcr's secret exchange carries it here, so two processes hold a copy for the length of the login. The exchange is an ephemeral Diffie–Hellman, so the plaintext is not in a bus message — but "never leaves the process" describes the `gtk` path and this backend's own handling, not the whole journey. See "Which process draws the PIN field" |
...
| Every mechanism and parameter is re-validated against the mechanism **and the key** | `src/broker/mechanism.c` | `tests/test-mechanism.c`, including the RSA-PSS salt that does not fit. The RFC 8017 `emLen >= hLen + sLen + 2` check is computed in 64 bits: in `gsize` the sum wraps on a 32-bit build for a salt near `G_MAXUINT32`, and a wrapped sum passes and sends `sLen = 0xFFFFFFFF` to the module. `tests/test-discovery-limits.c` holds the boundary values |
...
| The backend enforces the grant lifetime itself and tears the card session down when it expires, **on the monotonic clock like the frontend's own expiry** | `src/session-impl.c` | closed, expired and cancelled are re-checked before the device lock and again with it held. The expiry was taken from `g_get_real_time()`, so an NTP correction or a user setting the date moved a live grant's deadline by the step; `tests/test-broker-regrant.c` pins the clock |
...
| Shutdown emits `SessionInvalidated(backend_gone)` and flushes before exit; when the name changes hands the previous owner's pending calls are answered `owner_gone` and its grants are closed **without a signal**, because `owner_gone` is not in the `SessionInvalidated` vocabulary and the successor knows no such session | `src/main.c`, `src/certificate-impl.c` | `tests/test-impl-dbus.c` asserts the shutdown reason; the reason is checked against the interface's five in `src/session-impl.c` (`expired`, `token_removed`, `policy`, `backend_gone`, `error`) and anything else is a `g_critical()` sent as `error` |
...
| `Decrypt` | not on either interface, and unreachable. `src/broker/mechanism.c` and `src/broker/operations.c` still contain the decryption code that used to be reachable: `RSA_OAEP`-only mechanism validation, one indistinguishable error per failure (`tests/test-broker-decrypt.c`), and a 32-decryption-per-grant budget (`CERTIFICATE_MAX_DECRYPTS_PER_GRANT`). `tests/test-impl-dbus.c` asserts only that `Decrypt` is not advertised. PKCS#1 v1.5 decryption is refused by name in that dead code and never will be reachable either: it is a padding oracle over the card's key [[S49](SOURCES.md), [S50](SOURCES.md)] |
...
binary `meson test` runs. That replay runs **under ASan and UBSan only when the build was configured
with `-Db_sanitize`** (`meson setup build-asan -Db_sanitize=address,undefined -Db_lundef=false`); in
an ordinary build it checks return values and nothing else. Opt-in loading (below) keeps the code
out of processes that never asked for it. What does **not** answer it is the argument at the top of
this section: "the application could already call the portal itself" is about the portal's assets
and says nothing about the consumer's.
...
`GrantInvalidated` with reason `token_removed` and closes the Session. **Detection is a poll, not an
event**: `src/tokens/discovery.c` lists the slots every 2 s and a change must survive two
consecutive polls before anything is emitted, so the invalidation lags a removal by seconds and a
card taken out and put back between polls is never seen to have gone. What stops that card being
used under the old grant is the module's own session invalidation —
`CKR_SESSION_HANDLE_INVALID`/`CKR_DEVICE_REMOVED`, which this backend maps to `token_removed` at the
next call — not the watcher, and **only where a token session is already open**. Login is lazy, so a
grant that has not yet reached its first operation has no PKCS#11 session to be invalidated, and a
removal and reinsertion between polls leaves it in place; a *different* card is still refused, since
the token behind a grant is re-resolved by manufacturer, model, serial and label when the session is
opened. (Were there a facade, the appropriate PKCS#11 device or token error would go to its client
and the endpoint would be **poisoned** so it cannot silently rebind to a card inserted later.)
Reinsertion after a removal that was seen requires explicit reselection **even when the label and
slot number are identical**, because a label and a slot number prove nothing about which card is in
the reader. Tokens are identified by every stable attribute available — manufacturer,
...
3600 s and enforces it on the monotonic clock, so a clock change cannot prolong one. The backend's
own copy of the expiry (`src/session-impl.c`) is on the monotonic clock too.
