...
The available candidate is `webauth-service` — the sibling design sketch in the author's
`entra-token-helper` repository, working name `webauth-service`, interface
`io.github.sjtrotter.WebAuthentication1`. It performs one interactive web authentication transaction
in a WebKitGTK window it owns, and Entra ID sign-in on a smart-card tenant redirects to
`certauth.<authority>`, which challenges for a TLS client certificate that lives on a PIV card. It
...
`docs/decisions/0007-smartcard-service.md` is the mirror of this decision, made from the other
...
**`webauth-service` is the first and, for phase 1, only consumer.** The interface is designed against
its needs, and the two sketches are written in parallel so the protocol between them is negotiated
while both are still cheap to change.
...
`webauth-service` asks on behalf of an RDP client, which asked on behalf of a user. This service sees
only its immediate D-Bus peer.

Version 1 shows the immediate peer and **nothing else**, honestly: the dialog names
`webauth-service`, not Remmina. That is worse for the user, who cares about Remmina, and it is the
...
`webauth-service`'s own window names the origin and its own caller, so between the two windows the
user has the information. That is a mitigation, not a solution, and it is why this is listed as an
open problem rather than a design.
...
- **A protocol to agree between two sketches** — the option names, the scoping guarantees, the grant
  ...
  will ever be.
- **Trust flows both ways.** This service decides whether `webauth-service` may ask for a certificate
  at all; and `webauth-service` must pass through enough about its own caller for that decision to be
  made as honestly as the delegation problem allows.
