...
The available candidate is the sibling `xdg-desktop-portal-webauth` backend, described in
[README.md](../../README.md). What makes it the right first consumer is that it performs its one
interactive web authentication transaction in a WebKitGTK window it owns, so it is a caller of the
Certificate portal rather than a peer of it: Entra ID sign-in on a smart-card tenant redirects to
`certauth.<authority>`, which challenges for a TLS client certificate on a PIV card. It has that
requirement today, from real hardware, in a real tenant — an observation from this project's own
runs ([TESTING.md](../TESTING.md) §2.55 and the live sign-in of 2026-09-05), not a documented
behaviour anyone else has verified here.
