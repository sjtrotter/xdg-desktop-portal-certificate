...
- **Interface conformance.** Request object paths built exactly as documented, so a caller can
  subscribe before calling; `handle_token` handling; exactly one terminal `Response` per request;
  `Close()` producing no later success; unknown options rejected rather than ignored.
...
- **Redaction** (`src/log/redact.h`). The important test is the negative one: a fixture library error
