...
| `mechanism` | the portal-mechanism-to-`CKM_*` mapping, its parameter validation, the DigestInfo construction, the `MGF1-<hash>` spelling, the `CK_RSA_PKCS_OAEP_PARAMS` mapping and the per-operation mechanism split, and the raw-to-DER ECDSA re-encoding | nothing |
