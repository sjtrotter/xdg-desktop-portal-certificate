...
|---|---|---|
| `filter` | the purpose rules and `certificate_filter`, against nine real certificates in `fixtures/` — including the S/MIME pair, so that `email` matching a decryption-only certificate is checked against certtool's own output rather than against a mock of it | nothing |
...
an empty one), `server-auth-only`, `encipherment-only`, `expired-client-auth`, `not-yet-valid`,
`email-encipherment-only` (emailProtection with `keyEncipherment` and no `digitalSignature` — the
decryption half of an S/MIME pair, and what a PIV key-management slot holds) and
`email-sign-and-encipher` (both bits, the ordinary one-certificate S/MIME setup).
